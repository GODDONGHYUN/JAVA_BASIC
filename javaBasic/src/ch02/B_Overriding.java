...
class SubClass2 extends SuperClass1 {
	...
	// @Override 어노테이션 : 컴파일러에게 아래에 오는 메서드가 오버라이드 메서드라는 것을 명시 !
	// 가독성 ↑, 개발자의 실수를 방지 할 수 있음
	 @Override
	 void instanceMethod() {
		// super 키워드 : 자손클래스에서 조상 클래스의 필드 혹은 메서드에 접근 할 때 사용
		super.instanceMethod();  // -> 부모 클래스에 생성한 메서드에 접근 
