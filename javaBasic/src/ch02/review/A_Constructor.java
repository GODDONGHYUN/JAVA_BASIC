...
class ConstructorClass {
	...
	// 생성시에 실행 할 코드를 구현한 메서드, 일반적으로 인스턴스 변수 초기화 작업에 사용
	// 생성자를 개발자가 직접 작성하지 않으면 자바가 기본 생성자를 자동으로 생성
	
	// 규칙 :
	// 1. 클래스명과 동일한 이름을 가져야 함
	// 2. 반환타입을 지정하지 않음 -> 무조건 인스턴스를 반환
	// 3. 인스턴스를 생성 한 후에 사용이 불가능 함
	ConstructorClass () {       // 기본 생성자 -> NoArgumentConstructor
		// 아래와 같은 상황을 '의존성을 내부에서 주입한다' 라고 함
		// 의존성을 내부에서 주입하는 상황을 지양해야 함
		this.instanceVariable1 = 10;
		this.instanceVariable2 = 9.9;
	}  
	
	// 생성자도 메서드 이기 때문에 오버로딩이 가능 함 !
	...
	ConstructorClass (int instanceVariable1, double instanceVariable2) {
		// this() : 현재 클래스의 다른 생성자를 호출 하는 키워드
