...
// 예외는 실행은 됨
// 에러는 개발자가 직접처리하기 힘든 경우가 대부분이고 예외는 개발자가 직접 처리할 수 있음

// 예외처리: try-catch 구문을 통해서 처리 가능
// try { 예외가 발생할 가능성이 있는 구문)
// catch (예외 클래스 예외인스턴스) {특정 예외가 발생할 때 실행할 구문 }
public class A_Exception {
	
	// throws 를 통해서 호출부에 예외처리에 대한 강제성을 부여
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(10 / number);
			 ...
			 System.out.println("수학적 예외가 발생 했습니다.");
		...
		try {
			...
			e.printStackTrace();
		}  // throw는 일반 메서드로 실행 불가능 ! static으로 생성하거나 인스턴스 생성 후 이용해야함
		System.out.println("프로그램이 정상적으로 종료 됐습니다.");
	}
